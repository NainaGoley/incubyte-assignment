node{
    ...
    stage('Clone repository') {
        ...
        checkout([$class: 'GitSCM', branches: [[name: '*/main']], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[credentialsId: 'incubyte_github_token', url: "https://github.com/NainaGoley/${product}.git"]]])
    }

    stage('Build image') {
        echo 'Build the docker flask image'
        app = docker.build("goley/${product}")
    }
